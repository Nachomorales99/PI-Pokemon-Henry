...
let getPokemonsOfJohto = async () => {
	let johto = await axios.get(
		'https://pokeapi.co/api/v2/pokemon/?offset=151&limit=100',
	);

	let allPokemonOfJohto = johto.data.results;

	for (let pokemon of allPokemonOfJohto) {
...
module.exports = getPokemonsOfJohto;
